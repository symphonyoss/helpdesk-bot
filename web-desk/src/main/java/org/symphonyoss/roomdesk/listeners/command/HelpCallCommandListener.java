...
public class HelpCallCommandListener extends AiCommandListener {
    ...
    private void init() {
        ...
        AiCommand sendInfo = new AiCommand(Config.getString(WebBotConfig.ROOM_INFO), 0);
        sendInfo.addAction(new RoomInfoAction());
        sendInfo.addPermission(new IsHelpCall());

        AiCommand sendSummary = new AiCommand(Config.getString(WebBotConfig.HELP_SUMMARY), 0);
        ...
        AiCommand exit = new AiCommand(Config.getString(WebBotConfig.EXIT), 0);
